...
export function BotHubRetrainingPage({ hubId }: Props) {
  ...
  if (!hub && loading) {
    return <main className="page"><p className="bot-hub__notice">봇 허브 재학습 후보를 불러오는 중입니다...</p></main>;
  }
    ...
    <main className="page bot-hub">
      ...
      <section className="bot-hub__panel bot-hub__retraining">
        ...
        <div className="bot-hub__filters">
          <label>
            하위 봇
            <select value={memberBotId} onChange={(event) => setMemberBotId(event.target.value)}>
              <option value="">전체 하위 봇</option>
          ...
          <label>
            처리 결과
            <select value={resultKind} onChange={(event) => setResultKind(event.target.value as StudioHubRetrainingCandidate["result_kind"] | "")}>
        ...
        <div className="bot-hub__history-table" role="table" aria-label="봇 허브 재학습 후보 이력">
          <div className="bot-hub__history-head" role="row">
            <span>사용자 발화</span><span>하위 봇</span><span>처리 결과</span><span>채널</span><span>발생 일시</span><span>반영</span>
          </div>
          {candidates.map((candidate) => {
            ...
            return (
              <div key={candidate.queue_event_id} className="bot-hub__history-row" role="row">
                ...
                {retrainingHref ? <Link className="bot-hub__secondary" href={retrainingHref}>학습 반영</Link> : <span className="bot-hub__help">대상 없음</span>}
              </div>
            );
          })}

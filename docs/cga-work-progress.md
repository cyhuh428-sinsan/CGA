...
### 봇 생성 화면 구도 시안 기준 재배치

- 신산님이 제공한 캡처는 기능 참고가 아니라 화면 시안으로 보고 반영하도록 기준을 다시 고정했다.
- `01 봇 생성` 화면을 Aidot형 구도로 다시 배치했다.
  - 상단: `버전 관리` / 우측 `취소`, `확인`
  - 본문 상단: `기본 정보` / `AI 생성 항목` / `구조 요약` 3분할
  - 본문 하단: 화면 전체폭을 쓰는 큰 구조 선택판
- 폰트는 확대된 캡처 이미지 크기를 따라가지 않고, 기존 공통 기준(`brand 24 / top 16 / title 14 / body 12 / desc 10`)을 유지했다.
- 기존처럼 `AI 생성 항목`이 왼쪽 카드 내부 하단에 들어가 있던 구조를 제거하고, 캡처처럼 독립된 상단 중앙 패널로 분리했다.
- `구조 요약` 패널도 우측 좁은 고정 영역으로 옮기고, 하단 구조 선택판은 Aidot처럼 촘촘한 다열 보드로 재구성했다.

### 봇 관리 화면 재배치 보정

- 신산님이 제공한 봇 관리 기준 화면에 맞춰 `봇 관리` 화면의 버튼 배치를 보정했다.
- `봇 자산 / 버전` 영역은 Aidot의 `버전 관리` 구성을 기준으로 유지하도록 정리했다.
  - 상단 작업 버튼: `버전 추가`, `복사`, `삭제`, `버전 파일 업로드`, `버전 파일 다운로드`, `운영/해제`
...
- 신산님 요청에 따라 `모든 화면, 모든 버튼` 점검 보고 형식으로 정리하기 전에, 우선 실제 불만이 집중된 인증/사용자관리/그룹관리의 기본 작동을 다시 확인했다.
- 재검증 기준:
  - 로그인/회원가입 API
  - 승인 대기 로그인 차단
  - 사용자 저장/삭제 API
  ...
  - 그룹 생성 저장 동작
- 확인 결과:
  - `node scripts/check-auth-api.mjs` 재실행 통과
  - `node --check apps/studio/app.js` 통과
  - `node --check scripts/serve-studio.js` 통과
  - 로컬 실행 서버 기준에서 승인 대기 사용자는 로그인 차단 상태 유지
  - 사용자 수정/삭제용 `/api/cga/users/:userId` 경로와 그룹 조회 저장 로직은 서버/API 기준 정상 응답 확인
- 현재 보고서에 반영할 우선 점검 항목:
  - 로그인 화면
  - 회원가입 화면
  - 사용자 관리 화면
  - 그룹 관리 화면

### 인증 헤더/로그아웃 오염 수정 및 실제 버튼 재검증

- 로그아웃 후 메모리 상태를 다시 `admin` 으로 되돌리던 버그를 제거했다.
- 세션 토큰이 없는데도 `X-CGA-User-Id: admin` 헤더를 강제로 보내던 버그를 제거했다.
- 이 수정으로 로그인 화면 상태와 실제 인증 상태가 서로 어긋나던 문제를 먼저 차단했다.
- 검증:
  - `node --check apps/studio/app.js`
  - `node scripts/check-auth-api.mjs`
  - 브라우저 점검:
    - 로그인 버튼 정상 이동 확인
    - 로그아웃 버튼 정상 복귀 확인
    - 로그아웃 후 `#api-answer-source` 직접 진입 차단 확인
    - 사용자 관리 조회 버튼 필터링 확인
    - 사용자 상세 저장 버튼 상태 변경/복구 확인
    - 그룹 관리 조회 버튼 필터링 확인
    - 그룹 생성 버튼 오픈 확인
    - 그룹 생성 저장 확인
    - API 등록 버튼 다이얼로그 오픈 확인
    - API 더보기 버튼 메뉴 오픈 확인
...
- `봇스테이션` 본문도 `draft/ready` 같은 CGA 봇 상태로 화면을 숨기지 않고, Aidot 원본처럼 실제 채널 목록 기준으로 표를 먼저 보여주도록 수정했다.
  - 채널이 있으면 목록/상태 표를 표시
  - `설정정보`는 URL 원문 대신 Aidot 원본처럼 `저장됨 / 저장되지 않음`으로 표시
  - 상단 상태 문구도 `등록된 채널 n개`형 임의 안내 대신 `봇이 연결되었습니다.`로 정리
...
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - 루트 CSS 변수만 우선 조정해 전체 디자인이 동일한 비율로 확대되도록 변경
  - 이후 개별 화면에서 밀도/줄바꿈/높이 문제가 생기는 구간은 후속 보정 대상으로 분리

### 좌측 메뉴 제목/설명 간격 보정

- 요청:
  - 좌측 메뉴 카드에서 제목과 설명문이 너무 붙어 보여 가독성이 떨어짐
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - `봇 관리`, `봇 작업공간`, 워크플로우 카드 공통 레이아웃의 고정 행 높이를 제거
  - 제목/설명 사이에 `row-gap`을 추가해 설명문이 한 줄 아래로 자연스럽게 떨어지도록 조정
  - 기존 카드 크기를 과도하게 키우지 않도록 기본/축소 구간 값을 분리해 보정

### 좌측 메뉴 카드 높이 재조정

- 요청:
  - 폰트와 간격이 커진 만큼 좌측 메뉴 카드의 상하 높이도 함께 확대 필요
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - 관리 메뉴 카드 최소 높이를 `48px`로 상향
  - 워크플로우 카드 최소 높이를 `50px`로 상향
  - 축소 구간 카드도 `44px` 기준으로 함께 조정
  - 내부 상하 패딩을 늘려 텍스트가 카드 안에서 눌려 보이지 않도록 보정

### 로그인 버튼 폰트 확대

- 요청:
  - 로그인 화면의 `로그인` 버튼 글자를 더 크게 표시
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - `.primary-action` 공통 버튼에 `font-size: var(--font-top)` 적용
  - 버튼 높이는 유지하고 텍스트만 한 단계 키워 시인성 보정

### 좌측 메뉴 설명문 제거

- 요청:
  - 좌측 메뉴에서는 설명문구를 모두 제거하고 제목만 남김
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - 섹션 제목 아래 보조 설명문 숨김
  - 관리 메뉴 카드 설명문 숨김
  - 워크플로우 카드 설명문 숨김
  - 워크플로우 접힘 요약 카드 설명문 숨김
  - 2줄 카드 레이아웃을 1줄 제목 중심 레이아웃으로 재정렬

### 저장 버튼 위치 통일

- 요청:
  - 저장 버튼은 모든 화면에서 화면 제목 바 오른쪽 끝에 오도록 통일
- 반영 파일:
  - `apps/studio/app.js`
  - `apps/studio/index.html`
  - `apps/studio/styles.css`
- 반영 내용:
  - 공통 화면 셸(`renderWorkflowScreenShell`)에 저장 버튼 액션 영역 추가
  - 각 화면 헤더 오른쪽 끝에서 동일한 위치로 저장 동작 실행
  - 상단 전역 바의 저장 버튼 제거

### 상단 언어 선택 정렬 보정

- 요청:
  - 상단 언어 라벨과 선택 박스의 높이/정렬이 어긋나 보이는 문제 수정
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - 언어 라벨을 독립 칩 형태로 변경
  - 라벨과 셀렉트 박스의 높이를 동일하게 `32px`로 통일
  - 내부 패딩과 보더를 맞춰 상단바 정렬 일관성 보정

### 공통 보조 버튼 외형 기준 지정

- 요청:
  - `로그아웃` 등 기본 버튼 외형을 `봇 복사` 버튼과 같은 모양으로 통일
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - `.ghost` 공통 버튼 클래스를 흰 배경, 연한 보더, 둥근 모서리 기준으로 정의
  - 상단 `로그아웃` 버튼도 동일 기준을 상속하도록 `quick-auth-button` 높이/패딩 정리

### 상단 로그아웃 버튼 위치 조정

- 요청:
  - 상단 우측 영역에서 `로그아웃` 버튼을 가장 끝으로 이동
- 반영 파일:
  - `apps/studio/index.html`
- 반영 내용:
  - `context-bar` 내부 순서를 조정해 언어 선택 뒤 마지막 위치로 변경

### 상단 언어 묶음 수직 정렬 보정

- 요청:
  - 상단 `언어 / 한국어` 묶음이 다른 버튼보다 위로 떠 보이는 문제 수정
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - `language-picker` 바깥 컨테이너의 보더/패딩 제거
  - 언어 선택 박스 높이를 `32px`로 고정
  - 언어 묶음 전체를 상단 우측 요소들과 같은 중심선에 정렬

### 작업 화면 버튼 외형 공통화

- 요청:
  - `작업 봇 열기`, `봇 설정 열기`, `봇 구성 열기` 같은 작업 화면 버튼도 공통 버튼 외형으로 통일
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - `command-panel` 헤더 버튼, `command-action-stack` 버튼, `package-action-grid` 버튼, `workspace-command-footer` 버튼을
    모두 동일한 흰 배경/연한 보더/둥근 모서리 버튼 규칙으로 통일
  - 비활성 상태 투명도 규칙도 동일하게 정리

### 공통 폰트/버튼 디자인 기준 2차 통일

- 요청:
  - 전체 화면의 폰트와 버튼 모양을 더 넓은 범위에서 공통 기준으로 통일
- 반영 파일:
  - `apps/studio/styles.css`
- 반영 내용:
  - 공통 컨트롤 토큰(`height/radius/padding`) 추가
  - `ghost`, `secondary-action`, `primary-action`, admin/workspace/table 계열 버튼을 공통 흰 배경 버튼 체계로 통일
  - 관리 화면과 API 저장소 화면의 로컬 폰트 크기를 공통 `--font-body` 기준으로 정리

### 예시성 기본값 2차 제거

- 요청:
  - 화면에 박혀 있는 가짜 데이터/예시 기본값 제거
- 반영 파일:
  - `apps/studio/app.js`
- 반영 내용:
  - Semantic Vector / Semantic RAG 선택 시 인덱스 이름 기본 예시값 자동 주입 제거
  - 대화 설계 화면의 `Validation Set 상태: Random` 고정 문구를 실제 현재 설정값 기반 표기로 변경
...
### 로그인/회원가입 진입 화면 디자인 보정

- 요청:
  - 회원가입 화면이 현재 전체 컨셉과 맞지 않고 폼 카드 구조가 어색함
- 반영 파일:
  - `apps/studio/index.html`
  - `apps/studio/styles.css`
- 반영 내용:
  - 회원가입 패널 상단에 로그인 카드와 동일한 브랜드 워드마크를 추가
  - 회원가입 카드를 별도 레이아웃으로 정의해 폭, 패딩, 헤더 간격, 폼 간격을 재정리
  - `서버 / 그룹` 영역을 2열 메타 그리드로 정리하고 모바일에서는 1열로 자동 전환
  - textarea, 하단 전환 문구, 신청 버튼 간격을 로그인 카드 톤과 맞게 정리
  - 비밀번호 / 비밀번호 확인 입력을 2열 필드 그리드로 정리해 세로 길이를 줄이고 폼 밀도를 개선
  - 헤더/푸터 구분선을 추가하고 카드 그림자/폭을 재조정해 로그인 진입 카드와 동일 계열의 디자인으로 재정렬

### 기본 작동 우선 보정 진행

- 요청:
  - 승인 대기 사용자는 로그인되면 안 됨
  - 동일 사용자 ID 가입은 차단되어야 함
  - 사용자 관리/그룹 관리 `조회` 버튼이 실제 재조회 동작을 해야 함
  - 그룹 관리 `+ 그룹 생성` 버튼이 실제 생성 흐름으로 연결되어야 함
- 반영 파일:
  - `apps/studio/app.js`
  - `apps/studio/i18n.js`
  - `packages/public-core/src/access-state.js`
  ...
  - `scripts/check-auth-api.mjs`
- 반영 내용:
  - 회원가입 시 사용자를 즉시 활성화하지 않고 `pending` 상태로 생성되도록 변경
  - 가입 승인 시에만 사용자 상태가 `active`로 전환되도록 보정
  - 승인 전 로그인 시 `CGA_PENDING_APPROVAL` 오류를 반환하도록 서버 인증 로직 보강
  - 회원가입 성공 시 세션을 즉시 발급하지 않도록 수정하고 `승인 대기` 안내 메시지를 노출
  - 네트워크 오류 시 로컬 fallback으로 강제 로그인/강제 회원가입되던 우회 경로 제거
  - 사용자 관리 목록에서 동일 사용자 ID가 중복 membership으로 두 번 표시되지 않도록 사용자 ID 기준 병합 처리
  - 사용자 관리/그룹 관리 `조회` 버튼 클릭 시 서버 재조회 후 목록을 다시 렌더링하도록 연결
  - 그룹 관리 `+ 그룹 생성` 버튼이 생성 모드 팝업을 열고 실제 저장까지 이어지도록 연결
- 검증:
  - `node --check apps/studio/app.js`
  - `node --check scripts/serve-studio.js`
  - `node scripts/check-auth-api.mjs`
  - 로컬 브라우저에서 `사용자 관리` 검색값 `u-builder` 조회 시 단일 사용자만 남는 상태 확인
  - 로컬 브라우저에서 `그룹 관리`의 `조회`, `+ 그룹 생성` 버튼이 실제 반응하는 상태 확인

### 인증/권한/그룹/API 화면 버튼 실재검증

- 요청:
  - 모든 화면의 기본 버튼이 실제로 동작하는지 다시 점검
  - 특히 `로그인`, `로그아웃`, `회원가입`, `사용자 관리`, `그룹 관리`, `API` 화면의 기본 버튼을 실제 클릭 기준으로 확인
- 반영 파일:
  - `apps/studio/app.js`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 인증 헤더가 세션 없이 `admin`으로 전송되던 오염을 제거해 로그아웃 후 직접 화면 진입이 다시 로그인 화면으로 막히도록 보정
  - 로그아웃 시 메모리 상태가 다시 `admin`으로 복귀하던 문제를 제거해 실제 로그아웃 상태가 유지되도록 수정
  - 실제 브라우저에서 아래 흐름을 재검증
    - `admin / admin` 로그인
    - 로그아웃
    - 로그아웃 후 `#api-answer-source` 직접 진입 차단
    - 기존 ID 중복 회원가입 차단
    - 승인 대기 사용자 로그인 차단
    - 사용자 관리 `조회 / 초기화 / 저장 / 승인 / 삭제`
    - 그룹 관리 `조회 / 초기화 / + 그룹 생성 / 저장`
    - API `목록 / 등록 / 저장 / 수정`
- 검증:
  - `node --check apps/studio/app.js`
  - `node scripts/check-auth-api.mjs`
  - 로컬 브라우저에서 `로그인 -> 로그아웃 -> 직접 진입 차단` 흐름 확인
  - 로컬 브라우저에서 `u-builder` 중복 회원가입 시 `이미 존재하는 사용자 ID입니다.` 메시지 확인
  - 로컬 브라우저에서 승인 대기 사용자 로그인 시 `관리자 승인 후 로그인할 수 있습니다.` 메시지 확인
  - 로컬 브라우저에서 임시 승인 대기 사용자를 `승인 -> 삭제`까지 완료 확인
  - 로컬 브라우저에서 임시 API를 `등록 -> 수정`까지 완료 확인

### 2026-06-30 추가 실클릭 재점검

- 요청:
  - 화면별 버튼을 다시 실제 클릭 기준으로 점검
  - 특히 로그인, 사용자 관리, 그룹 관리의 기본 버튼 동작을 다시 확인
- 반영 파일:
  - `apps/studio/app.js`
  - `packages/public-core/src/access-state.js`
  - `scripts/serve-studio.js`
  - `docs/cga-work-progress.md`
- 확인 내용:
  - 로그인 화면에서 `data-entry-login-submit` 바인딩이 다시 살아있는지 확인
  - `admin / admin` 로그인 클릭 후 실제로 `#workspace-home`으로 전환되는 상태 확인
  - 사용자 관리 `조회`, `초기화` 버튼이 목록을 다시 필터링/복원하는지 확인
  - 사용자 상세에서 `계정 상태 active -> inactive -> active` 저장이 실제 반영되는지 확인
  - 그룹 관리 `조회`, `초기화`, `+ 그룹 생성`, `삭제` 버튼이 실제 반응하는지 확인
- 검증:
  - 로컬 브라우저에서 로그인 클릭 후 `#workspace-home` 전환 확인
  - 로컬 브라우저에서 사용자 관리 `cyhuh` 검색 후 `전체 1건`으로 축소 확인
  - 로컬 브라우저에서 사용자 관리 초기화 후 `전체 7건` 복원 확인
  - 로컬 브라우저에서 `cyhuh` 계정 상태를 `비활성` 저장 후 상세 패널 반영 확인
  - 로컬 브라우저에서 `cyhuh` 계정 상태를 `활성`으로 원복 후 상세 패널 반영 확인
  - 로컬 브라우저에서 임시 그룹 `g-eoul-1782749064328` 선택 후 삭제 시 검색 결과 `전체 0건` 확인
  - 로컬 브라우저에서 그룹 초기화 후 삭제된 임시 그룹이 기본 active 목록에서 제외된 상태 확인
- 남은 확인:
  - API 화면은 현재 선택 봇 컨텍스트가 비어 있을 때 목록/등록 반영이 혼선이 있어, 봇 컨텍스트를 고정한 상태에서 `등록/상세/수정/삭제/업로드/다운로드`를 다시 점검해야 함

### 2026-06-30 인증 라우팅 우선순위 수정

- 문제:
  - `/api/cga/groups/join-requests` 경로가 일반 그룹 상세 경로보다 뒤에서 해석되어 `405 Method Not Allowed`로 잘못 빠지는 문제가 있었다.
  - 이 상태에서는 가입 승인/대기 흐름 자동 점검이 실패하고, 기본 인증 검증 결과도 신뢰하기 어려웠다.
- 반영 파일:
  - `scripts/serve-studio.js`
- 수정 내용:
  - `parseAuthApiPath()`에서 `join-requests`, `join-requests/:id/approve`를 일반 `groupRecord`보다 먼저 해석하도록 순서를 조정했다.
- 검증:
  - `node --check scripts/serve-studio.js`
  - `node scripts/check-auth-api.mjs`
  - 결과: `OK auth and group API endpoints passed`

### 2026-06-30 봇 생성/버전 생성 의미 재정렬

- 요청:
  - `봇 생성`과 `버전 생성`의 의미를 분리
  - `봇 작업공간`은 생성 화면이 아니라 기존 봇/버전 작업 공간으로 고정
- 반영 파일:
  - `apps/studio/app.js`
  - `apps/studio/index.html`
  - `apps/studio/data/workflow.js`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - `봇 생성 = 새 봇 생성 + 최초 버전 v0.1 생성`
  - `버전 생성 = 기존 봇에 후속 버전 추가`
  - `봇 작업공간` 설명과 빈 목록 안내 문구를 생성 공간이 아닌 작업 공간 기준으로 수정
  - 작업공간에서 사용하지 않는 즉시 봇 생성 잔여 코드 제거
  - `봇 생성` 화면의 버전/자산 안내 문구를 최초 버전 생성 규칙 기준으로 수정
- 검증:
...
### 2026-06-30 봇 생성 버전관리 팝업 정렬

- 요청:
  - `봇 생성` 화면의 `버전 관리` 버튼을 누르면 Aidot 스타일의 버전관리 팝업이 열려야 함
- 반영 파일:
  - `apps/studio/index.html`
  - `apps/studio/app.js`
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - `봇 생성` 상단 툴바에 있는 `버전 관리` 버튼 클릭 시 기존 `봇 관리` 팝업이 아니라 전용 `버전 관리` 팝업을 열도록 연결 수정
  - `봇 생성` 상단 `취소`, `확인` 버튼 동작 연결
  - Aidot 버전관리 화면과 같은 용도의 전용 팝업 레이아웃/버튼/표/하단 닫기 영역 CSS 추가
  - 기존 생성 화면 하단의 중복 `버전 및 자산` 카드 제거 상태 유지

### 2026-06-30 봇 생성 하단 조합표 연동

- 요청:
  - `봇 생성` 화면 하단 영역은 Aidot의 `봇/봇 허브 생성` 하단 조합표처럼 보여야 함
  - 위에서 선택한 `NLU 방식`, `NLU 모델`, `답변 방식`이 아래 조합표의 현재 선택 상태에 반영되어야 함
- 반영 파일:
  - `apps/studio/app.js`
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 기존 임시 카드형 `create-structure-card` 렌더링을 제거
  - Aidot식 하단 조합 매트릭스 구조로 변경
  - `ML`, `Semantic - Vector Worker`, `Semantic - External Embedding`, `LLM Engine` 행과
    `정해진 답변`, `Semantic Engine RAG`, `LLM Engine`, `LLM Engine RAG` 열 기준으로 조합표 구성
  - 위쪽 선택값과 동일한 교차 셀은 `실행/학습 가능 / 현재 선택` 상태로 강조
  - 선택 가능 조합은 `설정 전 / 선택 가능`, 불가 조합은 `사용 못함 / 사용 불가`로 표시

### 2026-06-30 생성 화면 버튼 정렬 및 표준 버튼 모양 통일

- 요청:
  - `버전 관리` 버튼은 생성 제목 옆으로 이동
  - 버튼 모양은 `+ 봇 생성` 기준의 둥근 표준 버튼 모양으로 통일
- 반영 파일:
  - `apps/studio/index.html`
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - `버전 관리` 버튼을 생성 화면 제목 줄 안으로 이동
  - 생성 툴바에는 `취소`, `확인` 버튼만 유지
  - 전역 `button` 기본 스타일을 둥근 테두리, 흰 배경, 회색 외곽선 기준으로 재정의
  - 생성 화면 상단 버튼 너비를 다시 맞춰 표준 버튼 비율에 맞춤

### 2026-06-30 생성 화면 구조 요약 폰트 표준 복원

- 요청:
  - `구조 요약` 영역의 폰트가 너무 작지 않게 표준 크기를 유지해야 함
- 반영 파일:
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - `구조 요약` 헤더 우측 상태 문구 폰트를 `font-desc`에서 `font-body`로 상향
  - 요약 항목 라벨과 값 폰트를 모두 `font-body` 기준으로 조정
  - 줄간격도 함께 보정해서 작은 글씨처럼 보이지 않게 수정

### 2026-06-30 생성 화면 라벨 스타일 정렬

- 요청:
  - `구조 요약`의 `언어` 같은 라벨이 왼쪽 입력 라벨과 같은 기준으로 보여야 함
- 반영 파일:
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - `구조 요약` 라벨(`b`)의 색상과 굵기를 왼쪽 입력 라벨 기준으로 상향
  - 값(`span`)도 본문 표준 굵기로 맞춰서 전체 균형을 정리

### 2026-06-30 생성 화면 구조 요약 값 글자 크기 보정

- 요청:
  - `구조 요약`의 값(`ko`, `ml`, `fixed`)이 왼쪽 선택박스 값과 같은 체감 크기로 보여야 함
- 반영 파일:
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - `구조 요약` 값(`span`) 글자 크기를 `font-body`에서 `font-title`로 상향
  - 오른쪽 요약 패널 값이 작아 보이던 문제를 보정

### 2026-06-30 생성 화면 상단 폭 비율 재조정

- 요청:
  - `AI 생성 항목` 영역의 좌우 폭을 더 키우고
  - 그만큼 `구조 요약` 폭을 줄여야 함
- 반영 파일:
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 상단 3분할 `create-top-grid` 비율을 다시 조정
  - 가운데 `AI 생성 항목` 컬럼 폭을 확대
  - 오른쪽 `구조 요약` 최소 폭과 비율을 축소

### 2026-06-30 생성 화면 세로 스크롤 높이 재조정

- 요청:
  - 상단 영역이 커지면서 생성 화면에 불필요한 세로 스크롤이 생기지 않도록 조정해야 함
- 반영 파일:
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
...
### 2026-06-30 봇 생성 확인/마지막 버전 삭제 동작 보강

- 요청:
  - 기존 봇 컨텍스트의 봇 생성 화면에서는 `확인`이 비활성화되어야 함
  - 버전 관리에서 버전이 1개뿐인 경우 `삭제`는 마지막 버전 삭제가 아니라 봇 삭제로 처리하고, 재확인을 받아야 함
- 반영 파일:
  - `apps/studio/app.js`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 기존 봇이 선택된 상태의 봇 생성 화면에서는 하단 `확인`과 상단 공통 저장 버튼이 모두 비활성화되도록 보강
  - 하단 `확인` 클릭 이벤트에도 저장 실행 전 방어 조건을 추가
  - 마지막 버전 삭제 시 기존 확인 문구를 유지하고, 서버 삭제 성공 후에도 로컬 봇 목록/버전 레지스트리가 즉시 제거되도록 보강

### 2026-06-30 봇 생성 화면 신규 봇 생성 버튼 추가

- 요청:
  - 봇 생성 화면 제목 옆에 `+ 봇 생성` 버튼 추가
  - 신규 봇 생성 시 기존 봇의 봇 이름/소개가 남지 않아야 함
  - AI 생성 항목과 구조 요약은 초기값으로 표시되어야 함
- 반영 파일:
  - `apps/studio/index.html`
  - `apps/studio/app.js`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - `버전 관리` 버튼 옆에 `+ 봇 생성` 버튼 추가
  - 버튼 클릭 시 현재 선택 봇 컨텍스트를 해제하고, 봇 이름/소개를 빈 값으로 초기화
  - AI 생성 항목 초기값을 `LLM Engine`, `LLM Engine RAG 답변`, `ChatGPT`, `GPT-4o mini`, `v0.1` 기준으로 표시
  - 브라우저 캐시 반영 문제를 줄이기 위해 Studio 정적 파일 query version을 갱신

### 2026-06-30 봇 생성 화면 버전 기능 동작 확인/보강

- 요청:
  - 봇 생성 화면의 `신규봇 생성`, `버전 추가`, `봇(버전) 복사`, `봇(버전) 삭제`, `버전 파일 다운로드`, `버전 파일 업로드` 동작 확인
  - Aidot 실제 동작 기준으로 확인 절차를 맞출 것
  - 단, CGA는 마지막 버전 삭제 시 봇 삭제 정책을 유지하고 재확인 질문을 표시해야 함
- 반영 파일:
  - `apps/studio/app.js`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 저장되지 않은 새 봇 draft 상태에서는 버전 기능 실행 전 `봇을 먼저 생성한 후 버전 기능을 사용할 수 있습니다.` 안내 표시
  - `버전 추가` 실행 전 Aidot 기준 확인 질문 추가
  - `복사` 실행 전 Aidot 기준 확인 질문 추가
  - `버전 파일 다운로드` 실행 전 Aidot 기준 확인 질문 추가
  - `버전 파일 업로드`는 저장된 봇이 있을 때만 파일 선택 절차로 진입하도록 보강
  - 마지막 버전 삭제는 CGA 정책대로 봇 삭제 재확인 후 처리하는 기존 동작 유지
  - 생성 화면의 `버전 관리` 버튼이 초기 화면 로딩 직후에도 동작하도록 부팅 시 workspace action 바인딩을 추가

### 2026-06-30 봇 생성 화면 버전 기능 모달 스타일 보강

- 요청:
  - Aidot 실제 동작처럼 `버전 추가`, `복사`, `버전 파일 업로드`, `버전 파일 다운로드` 실행 전에 확인/입력 모달이 보여야 함
  - 버튼은 CGA 표준 버튼 형태를 유지해야 함
- 반영 파일:
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 버전 기능 확인 모달(`version-action-modal`)의 헤더/본문/행/파일 선택/하단 버튼 스타일 추가
  - 버전 관리 팝업 내부 버튼을 표준 둥근 버튼 형태로 정리
  - 삭제는 데이터 삭제 위험이 있어 자동 확정 테스트 대상에서 제외하고, CGA 정책의 재확인 질문은 유지

### 2026-06-30 봇 생성 화면 버전 관리 버튼 실제 동작 검증/수정

- 요청:
  - 내가 하던 확인을 Codex가 직접 수행하고, 변경 결과를 확인한 뒤 보고할 것
- 확인 결과:
  - `+ 봇 생성` 클릭 시 상단 봇 컨텍스트가 `없음`으로 변경되고, 봇 이름/소개가 빈 값으로 초기화됨
  - AI 생성 항목과 구조 요약은 초기값(`LLM Engine`, `LLM Engine RAG`, `ChatGPT`, `GPT-4o mini`, `v0.1`)으로 표시됨
  - 신규 봇 상태에서는 `확인` 버튼이 비활성화됨
  - 버전 관리 팝업의 `버전 추가`, `복사`, `버전 파일 업로드`, `버전 파일 다운로드` 버튼 클릭 시 각각 Aidot 기준 확인/업로드 모달이 표시됨
- 추가 수정:
  - 팝업 내부 `버전 추가`, `버전 파일 업로드`, `버전 파일 다운로드` 버튼이 첫 번째 DOM 버튼에만 이벤트 바인딩되는 문제를 수정
  - 확인 모달을 띄우는 즉시 버전 관리 팝업이 자동 재생성되어 모달이 중복될 수 있는 즉시 새로고침 속성을 제거
  - 실제 삭제는 데이터 삭제 위험이 있어 자동 확정하지 않았고, CGA 정책의 마지막 버전 삭제 재확인 흐름은 유지

### 2026-07-01 버전 복사 생성 버전 번호 수정

- 요청:
  - `버전 복사`는 `v0.1-copy`가 아니라 다음 버전 번호인 `v0.2`로 복사되어야 함
- 반영 파일:
  - `apps/studio/app.js`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 버전 복사 확인 모달의 `생성 버전` 표시를 `buildNextBotVersionId()` 기준으로 변경
  - 실제 복사 생성 로직도 `-copy` 접미어 대신 다음 버전 번호를 사용하도록 변경

### 2026-07-01 버전 파일 업로드 모달 Aidot 레이아웃 보정

- 요청:
  - CGA `버전 파일 업로드` 모달 화면이 Aidot 업로드 모달과 다르게 깨져 보임
- 반영 파일:
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 업로드 모달 폭을 Aidot 기준에 맞춰 확장
  - `새로운 버전으로 추가`, `기존 버전 덮어쓰기` 선택지를 전체 폭 카드형 행으로 정렬
  - 파일 선택 영역을 라벨/입력 영역으로 정렬해 Aidot 화면과 유사하게 표시

### 2026-07-01 버전 관리 팝업 선택/갱신 동작 보강

- 요청:
  - 버전 관리 팝업에서 버전 행 선택이 되지 않음
  - `버전 추가`, `복사`, `삭제`, `버전 파일 업로드`, `버전 파일 다운로드` 실행 후 팝업 내용이 최신 상태로 변경되어야 함
- 반영 파일:
  - `apps/studio/app.js`
  - `apps/studio/styles.css`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 버전 관리 팝업의 선택 컬럼을 실제 radio input으로 변경
  - 버전 행 클릭 시 작업 버전이 해당 버전으로 전환되고 팝업이 최신 상태로 다시 표시되도록 수정
  - 버전 추가/복사/삭제/업로드/다운로드/운영 전환 후 버전 관리 팝업을 최신 상태로 재렌더링하도록 보강
  - 운영 브라우저 캐시가 이전 `app.js/styles.css`를 사용하지 않도록 Studio 정적 파일 query version을 `20260701-1`로 갱신

### 2026-07-01 봇 생성 서버 저장/봇 관리 목록 반영 수정

- 요청:
  - `https://cga.sinsan.kr/#bot-management` 기준으로 생성된 봇이 봇 관리 화면에 보여야 함
  - `현재 작업 내용을 로컬에 저장했습니다.` 같은 땜빵성 Message 배너를 표시하지 말 것
- 반영 파일:
  - `apps/studio/app.js`
  - `apps/studio/index.html`
  - `docs/cga-work-progress.md`
- 반영 내용:
  - 신규 봇 생성 화면의 `확인` 버튼이 `createWorkspaceBotOnServer()`를 호출하도록 연결
  - 신규 봇 생성 성공 후 서버 봇 목록을 재조회하고, 생성된 봇을 현재 작업 봇/봇 관리 선택 봇으로 반영
  - 봇 이름은 중복 가능하지만 봇 ID는 같은 그룹 내에서 중복되지 않도록 자동 생성

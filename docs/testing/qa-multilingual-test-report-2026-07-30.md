...
**판정: 배포 가능.** 확정된 결함 1건은 분석 화면 도움말 툴팁의 미번역으로, 기능 장애가 아닌 번역 누락이다.
...
| **DB 통합 테스트 4건** | **조치 완료** | Alembic URL 마스킹·상대경로·로깅 격리 픽스처 결함 3건 수정(`bd9f6a4`, `6a0caf9`) |
...
### 5.1 [결함 · 중간] 분석 화면 도움말 툴팁 5종이 6개 언어 전부에서 한국어로 표시됨
...
### 5.2 [관찰 · 정보성] 언어 선택기의 접근성 이름이 선택값을 반영하지 않음

- **위치**: 로그인/회원가입 화면의 언어 선택 `<select>`
- **증상**: 일본어가 선택된 상태(`option "日本語" (selected)`)에서도 접근성 트리의 combobox 이름이 `"한국어"`로 보고된다.
- **판정**: 시각적 표시와 기능은 정상이며, 접근성 트리 계산 방식(라벨 없는 select가 첫 옵션 텍스트를 이름으로 취하는 경우)에 기인할 수 있다. **결함으로 확정하지 않는다.**
- **권고**: 스크린리더 사용자 지원이 요구사항에 포함된다면 `<select>`에 명시적 `aria-label`(언어별 번역)을 부여하는 것을 검토할 것. 요구사항이 아니라면 조치 불필요.
...
| 5.1 결함의 라이브 육안 확인 | 위와 동일. 분석 화면 진입에 로그인 필요 |
...
1. **[결함 수정]** `analysis.ts`의 5개 도움말 키를 6개 언어로 번역 (5.1절).
2. **[회귀 방지 — 중요]** 카탈로그 **값**의 미번역을 감시하는 테스트를 추가할 것. 현재 `satisfies`는 키만 보장하고, 팀의 한국어 리터럴 검사는 `.tsx`만 보므로 **`lib/i18n/*.ts` 내부는 어떤 자동 검사도 감시하지 않는다.** 5.1과 동일한 결함이 다시 유입될 수 있다.
   - 구현 제안: `lib/i18n/*.ts`를 로드해 비-ko 값에 한글이 남아 있으면 실패시키되, 아래 정당한 예외는 화이트리스트로 관리
     - CSV 헤더/형식 예시 (`uploadHeaderHelp`, `evaluation` `<code>` 예시)
     - 한국어 원문 키 패턴의 키 문자열 자체
3. **[커버리지 확대]** `test_multilingual_support_contract.py`의 대상 컴포넌트 14개 하드코딩 목록을 `components/*.tsx` 전체 스윕 + 예외 화이트리스트 방식으로 전환할 것. 현재는 신규 컴포넌트가 자동 감시되지 않는다(3.2절 분석).
...
5. **[후속 검증]** 로그인 세션 확보 후 인증 화면의 라이브 다국어 검증과 5.1 육안 확인을 수행할 것 (7장).

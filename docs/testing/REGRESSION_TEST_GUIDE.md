...
`apps/web`에 의존성이 설치되지 않은 환경에서는 다국어 카탈로그 검사 2건도 `skipped`로 표시된다. 백엔드만 검사하는 실행에서는 정상이며, 실제로 검사하려면 `apps/web`에서 `npm ci`를 먼저 수행하거나 `-Scope All`을 사용한다.
...
이 범위는 Web 빌드를 먼저 실행한다. `npm ci`가 `apps/web` 의존성을 맞춰 주어야 다국어 카탈로그 검사가 건너뛰지 않고 실제로 돌기 때문이다. 이어지는 pytest 실행에는 `CGA_REQUIRE_L10N_GUARD=1`이 설정되어, 도구가 없어 검사가 누락되면 `skipped`가 아니라 실패로 처리된다.

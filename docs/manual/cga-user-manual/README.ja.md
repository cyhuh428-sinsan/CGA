...
- エンティティは、発話から日付、地域、商品、注文番号などの値を抽出するために使用します。
- 辞書は業務用語と同義語を正規化します。変更後は関連するすべてのインテントを再テストします。
- 名前、代表表現、変数、接続する対話フローの整合性を保ちます。

## 11. 対話フローとテンプレート

1. インテントから開始カードを確認します。
2. 回答、条件、API、変数、次のカードを接続します。
3. 正常、例外、終了経路を作り、未接続カードがないことを確認します。
4. 対象チャネルで有効なテンプレートだけを選び、ボットテストと実チャネルで表示結果を確認します。

## 12. API の管理と実行

- API 名、送信先 Base URL、メソッド、認証、ヘッダー、Path、Query、Body を登録します。
- ブラウザーコードは same-origin の相対パスを呼び出し、Docker 内部アドレスと認証情報はサーバー側だけで扱います。
- テスト後、分析データと **Admin > API 呼び出し履歴** で状態、遅延、失敗理由を確認します。

## 13. ボットテスト、評価、再学習、分析

- ボットテストでは、回答と分析パネルの適用段階、Score、エンティティ、変数、対話カード、最終回答を同時に確認します。
- 評価では、同じ基準データで誤分類文、低 Score 文、類似インテント衝突を確認します。
- 再学習では確認済み発話だけを反映し、学習履歴で処理結果を確認します。
- 分析では、データ 0 件がフィルターや収集異常によるものかを区別してからモデル変更を判断します。

## 14. システム管理と運用ダッシュボード

- Admin でユーザー、グループ、権限、チャネル、Botstation 接続、テンプレート、ライセンス、多言語システムメッセージを管理します。
- 運用ダッシュボードで API/DB、キャッシュ、応答遅延、エラー、編集ロック、DB 分離、ML GPU、Semantic GPU の状態を確認します。
- 履歴画面でボット、バージョン、言語、発話、チャネル、要求時刻、結果を追跡し、認証情報や個人情報は露出しません。

## 15. 運用チェックリストと例

変更前にボット UUID、バージョン、言語、運用バージョン、影響資産、基準発話、接続チャネルを記録します。変更後は保存、学習・索引、ボットテスト、評価、分析、対話・API・Queue 履歴、実チャネルを確認します。

例：配送問い合わせボットを作り、配送予定と現在状態のインテント、多言語発話、注文番号エンティティ、API カードを登録し、学習または索引後に代表・境界発話をテストして、確認済みの失敗だけを再学習に反映します。
